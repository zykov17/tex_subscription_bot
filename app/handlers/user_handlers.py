    ...
    try:
        ...
        try:
            ...
            if is_subscribed:
                ...
                await message.answer(
                    f"👋 Привет, {first_name}!\n\n"
                    "Мы рады, что вы с нами! У вас есть возможность получить 4 катушки ниток в подарок при первом заказе. 🎁\n\n"
                    "Для этого нужно всего два простых шага:\n\n"
                    f"1️⃣ Подпишитесь на наш канал: <a href='https://t.me/{config.CHANNEL_USERNAME}'>Текстилия</a> — там мы публикуем новые коллекции, советы и эксклюзивные предложения.\n\n"
                    "2️⃣ После подписки нажмите кнопку 'Проверить подписку' ниже, чтобы получить ваш промокод на подарок\n\n"
                    "💡 Промокод действует только при первом заказе и даёт право на 4 катушки ниток. Не упустите шанс\n\n"
                    "Если возникнут вопросы, можете сразу написать менеджеру через кнопку ниже.",
        ...
        try:
            ...
            if is_subscribed:
                ...
                await callback.message.answer(
                    ...
                    f"Пожалуйста, подпишитесь на канал <a href='https://t.me/{config.CHANNEL_USERNAME}'>Текстилия</a> и нажмите кнопку проверки снова:",
                    reply_markup=create_subscription_keyboard(),
                    parse_mode="HTML"
                )
        finally:
...
async def handle_gift_delivery(message: Message, session, user_id: int, username: str, first_name: str):
    ...
    try:
        ...
        if gift_type == 'document':
            ...
            await message.answer(
                f"🎉 Ура, {first_name}!\n\n"
                f"Выш промокод на подарок: {gift_content}\n\n"
                "Спасибо, что подписались на наш канал💙\n\n"
                "Как воспользоваться промокодом?\n"
                "При оформлении заказа на сайте, введите прококод в комментарии к заказу.\n"
                "При оформлении заказа через менеджеров соцсетей, напишите промокод менеджеру",
                reply_markup=create_contacts_keyboard()
            )
...
@user_router.message()
async def unknowwn_message_handler(message: Message):
    """Фолбэк на любые непредусмотренные сообщения"""
    try:
        user_id = message.from_user.id
        username = message.from_user.username
        first_name = message.from_user.first_name

        logger.info(f"Пользователь {user_id} ввел непредусмотренныое сообщение: {message.text}")
        ...
        try:
            ...
            if is_subscribed:
                ...
                # Если не подписан - просим подписаться
                await message.answer(
                    f"🤖 Это ограниченный бот. Пожалуйста, используйте доступные кнопки и команды.\n\n"
                    "У вас есть возможность получить 4 катушки ниток в подарок при первом заказе. 🎁\n\n"
                    "Для этого нужно всего два простых шага:\n\n"
                    f"1️⃣ Подпишитесь на наш канал: <a href='https://t.me/{config.CHANNEL_USERNAME}'>Текстилия</a> — там мы публикуем новые коллекции, советы и эксклюзивные предложения.\n\n"
                    "2️⃣ После подписки нажмите кнопку 'Проверить подписку' ниже, чтобы получить ваш промокод на подарок\n\n"
                    "💡 Промокод действует только при первом заказе и даёт право на 4 катушки ниток. Не упустите шанс\n\n"
        ...
        logger.error(f"Ошибка в обработчике непредусмотренной команды: {e}")
        await message.answer("⚠️ Произошла ошибка. Пожалуйста, попробуйте позже.")

@user_router.callback_query()
async def unknowwn_callback_handler(callback: CallbackQuery):
    await callback.answer()
    await callback.message.answer(
        "⚠️ Это ограниченный бот. Пожалуйста, используйте кнопки, которые доступны в меню."
    )
